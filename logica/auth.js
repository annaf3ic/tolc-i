 ...
 * Usa Web Crypto API se disponibile (HTTPS/localhost),
 * altrimenti fallback a un hash semplice (sviluppo locale via file://).
 * @param {string} message
 * @returns {Promise<string>}
 */
async function sha256(message) {
  // Web Crypto API richiede contesto sicuro (HTTPS o localhost)
  ...
  // Fallback per file:// (sviluppo locale) — non sicuro per produzione
  return fallbackHash(message);
}

/**
 * Hash semplice per ambienti non-HTTPS (solo sviluppo).
 * @param {string} str
 * @returns {string}
 */
function fallbackHash(str) {
  ...
  // Rendi la stringa più lunga concatenando varianti per sembrare un hash
  ...
  return h; // 64 caratteri hex, come SHA-256
}
