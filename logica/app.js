...
async function loadQuestions() {
  try {
    ...
    if (!data.questions)
      throw new Error('Formato questions.json non valido: campo "questions" mancante');

    AppState.questions    = data.questions;
    // Il campo explanations è opzionale: presente nel formato grafo (nextPage),
    // assente nel formato flat (soluzione + feedback per ogni scelta).
 ...
 * @returns {Object|null}
 */
function findExplanation(page, currentQuestionId) {
  const matches = AppState.explanations.filter(e => e.sourcePage === page);
  if (matches.length === 0) return null;
  if (matches.length === 1) return matches[0];
  // Più spiegazioni sulla stessa pagina fisica: disambigua per questionId
  if (currentQuestionId) {
    const byQ = matches.find(e => e.questionId === currentQuestionId);
    if (byQ) return byQ;
  }
  return matches[0];
}
...
function renderQuestion(question) {
  ...
  // Immagine domanda — mostra se il campo 'image' è presente nel JSON
  const imgWrap    = document.getElementById('question-image-wrap');
  const imgEl      = document.getElementById('question-image');
  const imgMissing = document.getElementById('question-image-missing');
  if (imgWrap && imgEl && imgMissing) {
    if (question.image) {
      imgWrap.classList.remove('hidden');
      imgMissing.classList.add('hidden');
      imgEl.style.display = 'inline-block';
      imgEl.src = question.image;
      imgEl.onerror = () => {
        imgEl.style.display = 'none';
        imgMissing.classList.remove('hidden');
      };
      imgEl.onload = () => {
        imgEl.style.display = 'inline-block';
        imgMissing.classList.add('hidden');
...
/**
 * Risolve la "spiegazione" di una scelta supportando ENTRAMBI i formati:
 *
 * Formato FLAT (nuovo):
 *   choice.soluzione = true/false
 *   choice.feedback  = "testo spiegazione"
 *   → non serve il campo explanations nel JSON
 *
 * Formato GRAFO (vecchio):
 *   choice.nextPage → lookup in AppState.explanations per sourcePage + questionId
 *
 * @param {Object} choice       — oggetto scelta dal JSON
 * @param {string} questionId   — id della domanda corrente (per disambiguare il grafo)
 * @returns {{ isCorrect: boolean, text: string, nextPage: number|null }}
 */
function resolveExplanation(choice, questionId) {
  // ── Formato FLAT ──────────────────────────────────────────
  if (choice.soluzione !== undefined || choice.feedback !== undefined) {
    return {
      isCorrect: choice.soluzione === true,
      text:      choice.feedback  || '',
      nextPage:  null   // non usato nel formato flat
    };
  }
  // ── Formato GRAFO ─────────────────────────────────────────
  const exp = findExplanation(choice.nextPage, questionId);
  if (!exp) return null;
  return exp;
}
 ...
 * Supporta sia il formato flat (soluzione/feedback) sia il formato grafo (nextPage).
...
function handleChoiceClick(btn, choice) {
  ...
  const explanation = resolveExplanation(choice, AppState.currentQuestion?.id);

  // ── Simulazione: nessuna spiegazione, solo feedback visivo ───
  ...
  // Fallback se spiegazione non trovata (solo formato grafo)
  if (!explanation) {
  ...
  // Evidenzia scelta selezionata
  btn.classList.add(explanation.isCorrect ? 'correct' : 'wrong');

  // Se risposta sbagliata: evidenzia anche quella corretta
  if (!explanation.isCorrect) {
    const correctChoice = (AppState.currentQuestion.choices || []).find(c => {
      const exp = resolveExplanation(c, AppState.currentQuestion.id);
      return exp && exp.isCorrect;
    });
    if (correctChoice) {
      allBtns.forEach(b => {
        if (b.getAttribute('data-label') === correctChoice.label) b.classList.add('correct');
      });
    }
  }
 ...
 * Supporta sia il formato grafo (nextPage) sia il formato flat (avanza in sequenza).
 */
function advanceTraining(explanation) {
  const nextPage = explanation ? explanation.nextPage : null;

  // ── Formato FLAT: nessun nextPage → avanza alla domanda successiva in ordine ──
  if (!nextPage) {
    const questions = AppState.questions;
    const currIdx = questions.findIndex(q => q.id === AppState.currentQuestion?.id);
    const nextQ = questions[currIdx + 1];
    if (nextQ) {
      Auth.updateCurrentUser({ progressPage: nextQ.sourcePage });
